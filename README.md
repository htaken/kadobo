...
2. `[開始]` を押す → カードが一瞬 `⏳ 開始 HH:mm 記録中…`（この間はボタンが消える＝二度押し防止）→
   数秒で `✅ 開始 HH:mm 記録済み` に変わり、状態が `稼働中`（`[休憩][終了][✏️修正]`）になる
...
- **DM 通知**（`⚠️ 記録に失敗しました（処理ID: ... / エラー: ...）`）: `ok:false, retryable:false` として
  拒否された打刻。処理 ID を控え、必要なら操作をやり直す（自動では再送されない）。カードにも
  `⚠️ 記録に失敗しました（DM をご確認ください）` が出てボタンが戻る
- **本人だけに見える ephemeral**（`⚠️ 記録の反映を確認できませんでした（自動で再試行します）`）:
  GAS の応答が確認できず、記録されたかどうか不明な状態。このとき Worker は**カードを触らない**
  （押下時の古い表示で上書きすると、GAS がすでに描いた正しいカードを巻き戻してしまうため）。
  カードは `⏳ …記録中…` のまま残り、5 分周期の Cron 再送で GAS が正しい状態に描き直す。
  すぐ操作したいときは `/kado` で最新のカードを出し直す
